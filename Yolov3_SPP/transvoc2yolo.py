...
"""
  @Time    : 2024.01.10 15:35
  @File    : transvoc2yolo.py
  @Project : ObjectDet
  @Author  : Saitama
  @IDE     : PyCharm
  @Desc    : 把数据从PascalVOC格式转成YOLO格式
"""
import json
import os.path
import shutil
...
# PascalVOC数据集目录
# 原图片
voc_image_path = os.path.join(voc_root, voc_version, "JPEGImages")
# 标注数据
voc_xml_path = os.path.join(voc_root, voc_version, "Annotations")
# 训练txt
voc_train_txt_path = os.path.join(voc_root, voc_version, "ImageSets", "Main", train_txt)
# 验证txt
voc_val_txt_path = os.path.join(voc_root, voc_version, "ImageSets", "Main", val_txt)
# label标签对应的json文件
label_json_path = "./data/pascal_voc_classes.json"

# 检查上述目录是否都存在
assert os.path.exists(voc_image_path), "Pascal VOC image path doesn't exist"
assert os.path.exists(voc_xml_path), "Pascal VOC xml path doesn't exist"
assert os.path.exists(voc_train_txt_path), "Pascal VOC train txt path doesn't exist"
assert os.path.exists(voc_val_txt_path), "Pascal VOC val txt path doesn't exist"
...
def translate_info(file_names: list, save_root: str, class_dict: dict, train_val='train'):
    ...
    for file in tqdm(file_names, desc="translate {} file...".format(train_val)):
        # 检查下图像文件是否存在
        img_path = os.path.join(voc_image_path, file + ".jpg")
